...
async def list_daily_logs(
    ...
    query = """SELECT dl.*, p.name as project_name FROM daily_logs dl
              LEFT JOIN projects p ON p.id = dl.project_id WHERE dl.company_id=$1"""
    params = [company["id"]]
    if project_id:
        query += " AND dl.project_id=$2"; params.append(project_id)
    if from_date:
        query += f" AND dl.log_date >= ${len(params)+1}"; params.append(from_date)
    if to_date:
        query += f" AND dl.log_date <= ${len(params)+1}"; params.append(to_date)
...
async def get_daily_log(log_id: int, company=Depends(get_current_company), db=Depends(get_db)):
    row = await db.fetchrow(
        """SELECT dl.*, p.name as project_name FROM daily_logs dl
           LEFT JOIN projects p ON p.id = dl.project_id WHERE dl.id=$1 AND dl.company_id=$2""",
        log_id, company["id"]
    )
    if not row:
...
async def create_daily_log(body: DailyLogIn, company=Depends(get_current_company), db=Depends(get_db)):
    row = await db.fetchrow(
        """INSERT INTO daily_logs (company_id, project_id, log_date, weather, activities, issues, progress)
...
async def update_daily_log(log_id: int, body: DailyLogIn,
    ...
    row = await db.fetchrow(
        """UPDATE daily_logs SET project_id=$1, log_date=$2, weather=$3, activities=$4,
           issues=$5, progress=$6 WHERE id=$7 AND company_id=$8 RETURNING *""",
...
async def delete_daily_log(log_id: int, company=Depends(get_current_company), db=Depends(get_db)):
    await db.execute("DELETE FROM daily_logs WHERE id=$1 AND company_id=$2", log_id, company["id"])
    return {"ok": True}

@router.get("/by-project/{project_id}")
async def get_project_logs(project_id: int, company=Depends(get_current_company), db=Depends(get_db)):
    rows = await db.fetch(
        "SELECT * FROM daily_logs WHERE company_id=$1 AND project_id=$2 ORDER BY log_date DESC",
