...
async def update_payment_status(payment_id: int, status: str,
                                company=Depends(get_current_company), db=Depends(get_db)):
    row = await db.fetchrow(
        "UPDATE client_payments SET status=$1 WHERE id=$2 AND company_id=$3 RETURNING *",
...
async def payment_summary(company=Depends(get_current_company), db=Depends(get_db)):
    ...
    upcoming = await db.fetch(
        """SELECT cp.*, p.name as project_name
           FROM client_payments cp
           LEFT JOIN projects p ON p.id = cp.project_id
           WHERE cp.company_id=$1 AND cp.status='pending'
    ...
    overdue = await db.fetch(
        """SELECT cp.*, p.name as project_name
           ...
           LEFT JOIN projects p ON p.id = cp.project_id
           WHERE cp.company_id=$1 AND cp.status='pending'
           AND cp.due_date < CURRENT_DATE ORDER BY cp.due_date""",
        company["id"]
    )
    return {**dict(row), "upcoming_payments": [dict(r) for r in upcoming], "overdue_payments": [dict(r) for r in overdue]}

@router.get("/by-project/{project_id}")
async def get_project_payments(project_id: int, company=Depends(get_current_company), db=Depends(get_db)):
    rows = await db.fetch(
        "SELECT * FROM client_payments WHERE company_id=$1 AND project_id=$2 ORDER BY payment_date DESC",
